...
3D acceleration requires the VMware SVGA display drivers installed in the guest, but on modern Windows versions they usually will be automatically retrieved via Windows Update. You will also need a Vulkan-capable graphics card, and the `dxvk-native` package to be available on the host, preferably version 3.0 or higher.
